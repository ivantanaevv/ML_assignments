...
class Linear(Module):
    ...
    def update_grad_parameters(self, input: np.ndarray, grad_output: np.ndarray):
        ...
        :param grad_output: array of shape (batch_size, out_features)
        """
        self.grad_weight += np.transpose(grad_output) @ input
        if self.bias is not None:
...
class BatchNormalization(Module):
    ...
    def compute_output(self, input: np.ndarray) -> np.ndarray:
        ...
        if self.training:
            ...
            self.var = ((input - self.input_mean)**2).mean(axis=0)
            self.sqrt_var = np.sqrt(self.var +self.eps)
            self.inv_sqrt_var = 1 / self.sqrt_var
        
            self.norm_input = (input - self.input_mean) * self.inv_sqrt_var

            B = input.shape[0]
            corr = B/(B - 1) if B > 1 else 1
            self.running_var = (1 - self.momentum) * self.running_var + self.momentum * corr * self.var

            
            self.running_mean = (1 - self.momentum) * self.running_mean + self.momentum * self.input_mean
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        if self.affine:
            dx_hat = grad_output * self.weight
        else:
            dx_hat = grad_output

        N = len(input)

        dx = (1.0 / N) * self.inv_sqrt_var * (N *dx_hat-np.sum(dx_hat, axis=0)-self.norm_input * np.sum(dx_hat * self.norm_input, axis=0))

        return dx

        #взял отсюда (спасибо человеку гению) https://kevinzakka.github.io/2016/09/14/batch_normalization/ (закка закка ээе, ит ис африка)
...
class Dropout(Module):
    ...
    def compute_output(self, input: np.ndarray) -> np.ndarray:
        ...
        """
        if self.training:
            self.mask = np.random.binomial(1, 1 - self.p, size=np.shape(input))
...
class Sequential(Module):
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        for i in self.modules[::-1]:
            i.update_grad_parameters(self.inputs[-j], grad_output)
            grad_input = i.compute_grad_input(self.inputs[-j],grad_output )
